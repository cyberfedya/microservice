...
                {canSubmitForReview && isDocActive && (<button onClick={submitForReviewAction} disabled={isActionLoading} className="w-full flex items-center justify-center gap-2 px-4 py-2 text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:opacity-50"> <PaperAirplaneIcon className="w-5 h-5" /> Kelishuvga yuborish </button>)}
...
                    </div>
                )}
                {canSign && isDocActive && (<button onClick={signDocumentAction} disabled={isActionLoading} className="w-full flex items-center justify-center gap-2 px-4 py-2 text-white bg-purple-600 rounded-lg hover:bg-purple-700 disabled:opacity-50"> <PencilIcon className="w-5 h-5" /> Imzolash </button>)}
...
                        {/* --- ИЗМЕНЕНИЕ: Добавлена проверка window.confirm() --- */}
